...
import {
  Clock, Printer, Search, Calendar, User,
  Wrench, ClipboardCheck, FileText, X, Trash2, CheckCircle2,
} from 'lucide-react';
...
export function ServiceHistory() {
  ...
  const { machines, clearAllHistory } = useApp();
  ...
  const filteredEntries = useMemo(() =>
    allEntries.filter(entry => {
      ...
      if (dateRange.start) matchesDate &&= new Date(entry.timestamp) >= new Date(dateRange.start);
      if (dateRange.end)   matchesDate &&= new Date(entry.timestamp) <= new Date(dateRange.end + 'T23:59:59');
  ...
  const handleConfirmClear = () => {
    ...
    setToast('Histórico limpo com sucesso');
  };
          ...
          {Object.entries(groupedEntries).map(([date, entries]) => (
            <div key={date}>
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-gradient-to-br from-primary-500 to-secondary-500 rounded-xl flex items-center justify-center text-white font-bold text-sm shrink-0">
